...
if($mybb->input['action'] == "do_register" && $mybb->request_method == "post")
{
	...
	else
	{
		...
		else
		{
			...
			redirect("home.php", $lang->redirect_registered);
		}
	}
}
...
if($mybb->input['action'] == "activate")
{
	...
	if(isset($mybb->input['code']) && $user)
	{
		...
		elseif($activation['type'] == "b")
		{
			...
			redirect("home.php", $lang->redirect_accountactivated_admin, "", true);
		}
		else
		{
			$plugins->run_hooks("member_activate_accountactivated");

			redirect("home.php", $lang->redirect_accountactivated);
		}
	}
	else
	{
...
if($mybb->input['action'] == "do_resendactivation" && $mybb->request_method == "post")
{
	...
	else
	{
		if(count($errors) == 0)
		{
			...
			redirect("forums/index.php", $lang->redirect_activationresent);
		}
		else
		{
...
if($mybb->input['action'] == "do_lostpw" && $mybb->request_method == "post")
{
	...
	else
	{
		if(count($errors) == 0)
		{
			...
			redirect("forum/index.php", $lang->redirect_lostpwsent, "", true);
		}
		else
		{
...
if($mybb->input['action'] == "do_login" && $mybb->request_method == "post")
{
	...
	else if($validated && $loginhandler->captcha_verified == true)
	{
		...
		else
		{
			...
			redirect("home.php", $lang->redirect_loggedin);
		}
	}
...
if($mybb->input['action'] == "logout")
{
	...
	if(!$mybb->user['uid'])
	{
		redirect("forums/index.php", $lang->redirect_alreadyloggedout);
	}
	...
	redirect("forums/index.php", $lang->redirect_loggedout);
}
